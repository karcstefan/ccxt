...
class litebit(Exchange):
    ...
    async def fetch_markets(self, params={}):
        markets = await self.publicGetTradeMarket(params)
        results = []
    ...
    async def fetch_order_book(self, symbol, limit, params={}):
        request = {'code': symbol}
        if limit is not None:
            request.limit = limit
        orderBook = await self.publicGetTradeMarketCodeBook(request)
        results = []
    ...
    async def fetch_ticker(self, symbol, params={}):
        ...
        return await self.fetch_tickers([symbol], params)
    ...
    async def fetch_tickers(self, symbols=None, params={}):
        ...
        for i in range(0, len(self.markets)):
            ...
            if symbols is None or symbols.includes(marketData.id):
                ticker = await self.publicGetTradeMarketCodeHistory({'code': marketData.id})
                for j in range(0, len(ticker.data)):
                    data.append(ticker.data[j])
        return data

    async def fetch_balance(self):
        balances = await self.privateGetBalance()
        free = {}
        used = {}
        total = {}
    ...
    async def cancel_order(self, params={}):
        ...
        return await self.privateDeleteTradeOrderUuid(request)

    async def fetch_order(self, id, symbol=None, params={}):
        response = await self.privateGetTradeOfferUuid({'uuid': id})
        return self.transform_order_data(response.data)

    async def fetch_orders(self, symbol=None, since=None, limit=None, params={}):
        response = await self.privateGetTradeOffer()
        output = []
